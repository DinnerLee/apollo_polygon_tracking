...
public:
    ...
    bool init(caffe::Blob<float>* out_blob, bool use_constant_feature, int range, int width, int height);
    void generate(const pcl::PointCloud<pcl::PointXYZI> &pc_ptr);

    // raw feature data
    std::vector<bool> height_map_;
private:
    int width_ = 0;
    int height_ = 0;
    int range_ = 0;
