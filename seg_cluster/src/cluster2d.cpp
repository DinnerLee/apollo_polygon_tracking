                        ...
                        float objectness_thresh, bool use_all_grids_for_clustering)
{
    ...
    for (size_t i = 0; i < valid_indices_in_pc_->size(); ++i)
    {
        ...
        const auto &point = pc_ptr_[point_id];
...
void Cluster2D::getObjects(const float confidence_thresh,
                           ...
                           const std_msgs::Header &in_header, std::vector<bool> hm_data)
{
    ...
    for (size_t i = 0; i < point2grid_.size(); ++i)
    {
        ...
        if (obstacle_id >= 0)
        {
            if(hm_data.at(grid) == true){
                if (height_thresh < 0 ||
                        pc_ptr_[point_id].z <=
                        obstacles_[obstacle_id].height + height_thresh)
                {
                    ////////yep, we get the index!!!!!!!
                    obstacles_[obstacle_id].cloud_ptr->push_back(pc_ptr_[point_id]);
                    indices_.indices.push_back(point_id);
                }
            }
        }
        //        if (obstacle_id >= 0)
         ...
         obstacle_id++)
    {
        ...
        rockauto_msgs::DetectedObject out_obj = obstacleToObject(*obs, in_header);
